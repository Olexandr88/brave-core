...
TEST_F(BraveBrowserCommandHandlerTest, BasicCommandsExecuted) {
  TestingProfile::Builder builder;
  auto profile = builder.Build();
  auto& handler = CreateHandler(profile.get(), {Command::kOpenWalletOnboarding,
                                                Command::kOpenRewardsOnboarding,
                                                Command::kFocusLocationBar});
  ...
  handler->ExecuteCommand(Command::kOpenRewardsOnboarding, ClickInfo::New(),
                          base::DoNothing());
  handler->ExecuteCommand(Command::kFocusLocationBar, ClickInfo::New(),
                          future.GetCallback());
...
TEST_F(BraveBrowserCommandHandlerTest, OffTheRecordProfile) {
  ...
  ASSERT_TRUE(future.Get<0>());
  EXPECT_EQ(actions()[0], "execute-command: 39001");
}
