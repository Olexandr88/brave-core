...
class ZCashScanBlocksTask {
 public:
  using ZCashScanBlocksTaskObserver =
      base::RepeatingCallback<void(base::expected<ZCashShieldSyncService::ScanRangeResult, ZCashShieldSyncService::Error>)>;

  ZCashScanBlocksTask(ZCashShieldSyncService* sync_service, ZCashScanBlocksTaskObserver observer);
  ~ZCashScanBlocksTask();
 ...
 private:
  ...
  void OnGetAccountMeta(base::expected<ZCashOrchardStorage::AccountMeta, ZCashOrchardStorage::Error>
                            result);

  void GetChainTip();
  void OnGetChainTip(base::expected<zcash::mojom::BlockIDPtr, std::string> result);

  void PrepareScanRanges();

  void ScanRanges();
  void OnScanningRangeComplete(base::expected<bool, ZCashShieldSyncService::Error> result);
