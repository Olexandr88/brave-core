...
ZCashUpdateSubtreeRootsTask::ZCashUpdateSubtreeRootsTask(
    ZCashShieldSyncService* sync_service, ZCashUpdateSubtreeRootsTaskCallback callback) : sync_service_(sync_service), callback_(std::move(callback)) {
}
...
void ZCashUpdateSubtreeRootsTask::Start() {
  sync_service_->sync_state_.AsyncCall(&ZCashOrchardSyncState::GetLatestShardIndex)
...
void ZCashUpdateSubtreeRootsTask::OnGetLatestShardIndex(
    base::expected<std::optional<uint32_t>, ZCashOrchardStorage::Error> result) {
  if (!result.has_value()) {
  ...
  sync_service_->zcash_rpc_->GetSubtreeRoots(
      sync_service_->chain_id_, latest_shard_index + 1, kSubTreeRootsResolveBatchSize,
      base::BindOnce(&ZCashUpdateSubtreeRootsTask::OnGetSubtreeRoots,
                     weak_ptr_factory_.GetWeakPtr(), latest_shard_index + 1));
}

void ZCashUpdateSubtreeRootsTask::GetSubtreeRoots(
...
void ZCashUpdateSubtreeRootsTask::OnGetSubtreeRoots(
  ...
  sync_service_->sync_state_.AsyncCall(&ZCashOrchardSyncState::UpdateSubtreeRoots)
      .WithArgs(sync_service_->account_id_.Clone(), start_index, std::move(result.value()))
