...
class ZCashVerifyChainStateTask {
 public:
  using ZCashVerifyChainStateTaskCallback = base::OnceCallback<void(base::expected<bool, ZCashShieldSyncService::Error>)>;
  ZCashVerifyChainStateTask(ZCashShieldSyncService* sync_service, ZCashVerifyChainStateTaskCallback callback);
 ...
 private:
  ...
  void OnGetChainTipBlock(base::expected<zcash::mojom::BlockIDPtr, std::string> result);
