...
ZCashBlocksBatchScanTask::ZCashBlocksBatchScanTask(
  ZCashShieldSyncService* sync_service, uint32_t from, uint32_t to,
  ZCashBlocksBatchScanTaskCallback callback) : sync_service_(sync_service), from_(from), to_(to), callback_(std::move(callback)){
...
void ZCashBlocksBatchScanTask::GetFrontierTreeState() {
  auto block_id = zcash::mojom::BlockID::New(frontier_block_height_, std::vector<uint8_t>({}));
...
void ZCashBlocksBatchScanTask::OnGetFrontierTreeState(
  ...
  if (!result.has_value() || !result.value()) {
    error_ = ZCashShieldSyncService::Error{ ZCashShieldSyncService::ErrorCode::kFailedToReceiveTreeState, result.error() };
    ScheduleWorkOnTask();
    return;
  }
...
void ZCashBlocksBatchScanTask::OnGetFrontierBlock(
  base::expected<std::vector<zcash::mojom::CompactBlockPtr>, std::string> result) {
  if (!result.has_value() || result.value().size() != 1) {
    error_ = ZCashShieldSyncService::Error{ZCashShieldSyncService::ErrorCode::kFailedToDownloadBlocks, result.error()};
    ScheduleWorkOnTask();
    return;
  }
...
void ZCashBlocksBatchScanTask::DownloadBlocks() {
  uint32_t start_index = downloaded_blocks_ ? from_ + downloaded_blocks_->size() : from_;
...
void ZCashBlocksBatchScanTask::ScanBlocks() {
  if (!downloaded_blocks_ || downloaded_blocks_->empty()) {
    error_ = ZCashShieldSyncService::Error{ZCashShieldSyncService::ErrorCode::kScannerError, ""};
    ScheduleWorkOnTask();
    return;
  }

  if (!frontier_block_.value() || !frontier_tree_state_.value() || !frontier_block_.value()->chain_metadata) {
    error_ = ZCashShieldSyncService::Error{ZCashShieldSyncService::ErrorCode::kScannerError, ""};
    ScheduleWorkOnTask();
    return;
  }
...
void ZCashBlocksBatchScanTask::UpdateDatabase() {
  ...
  sync_service_->sync_state_.AsyncCall(&ZCashOrchardSyncState::UpdateNotes)
      .WithArgs(sync_service_->account_id_.Clone(), std::move(scan_result_.value()),
